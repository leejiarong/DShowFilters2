...
{
	{&__uuidof(CMpaDecFilter), L"Acer Audio Decoder", /*MERIT_DO_NOT_USE*/0x40000001, countof(sudpPins), sudpPins},
};

CFactoryTemplate g_Templates[] =
{
    {sudFilter[0].strName, &__uuidof(CMpaDecFilter), CreateInstance<CMpaDecFilter>, NULL, &sudFilter[0]},
	/*{L"CMpaDecPropertyPage", &__uuidof(CMpaDecSettingsWnd), CreateInstance<CInternalPropertyPageTempl<CMpaDecSettingsWnd> >},*/ //hide settings page
};
	...
	, m_boost(1)
{
	...
	CRegKey key;
	if(ERROR_SUCCESS == key.Open(HKEY_CURRENT_USER, _T("Software\\Acer\\Player\\Mpeg Audio Decoder"), KEY_READ))
	{
		DWORD dw;
...
{
	//CRegKey key;
	//if(ERROR_SUCCESS == key.Create(HKEY_CURRENT_USER, _T("Software\\Acer\\Player\\Mpeg Audio Decoder")))
	//{

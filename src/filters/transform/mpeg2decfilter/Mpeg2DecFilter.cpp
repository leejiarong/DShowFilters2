...
{
	#ifdef MPEG2ONLY
	{&__uuidof(CMpeg2DecFilter), L"Acer Video Decoder", 0x00600001, countof(sudpPins), sudpPins},
	#else
	{&__uuidof(CMpeg2DecFilter), L"Acer Mpeg Video Decoder", 0x00600001, countof(sudpPins), sudpPins},
	#endif
};
...
//	if(FAILED(*phr)) return;
	...
	CRegKey key;
	if(ERROR_SUCCESS == key.Open(HKEY_CURRENT_USER, _T("Software\\Acer\\Player\\Video Decoder"), KEY_READ))
	{
		DWORD dw;
...
{
	//CRegKey key;
	//if(ERROR_SUCCESS == key.Create(HKEY_CURRENT_USER, _T("Software\\Acer\\Player\\Video Decoder")))
	//{
